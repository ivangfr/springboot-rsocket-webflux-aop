...
if [ "$1" = "native" ];
then
  ./mvnw -Pnative clean spring-boot:build-image --projects "$MOVIE_SERVER_APP_NAME" -DskipTests="$SKIP_TESTS" -Dspring-boot.build-image.imageName="$MOVIE_SERVER_DOCKER_IMAGE_NAME"
  ./mvnw -Pnative clean spring-boot:build-image --projects "$MOVIE_CLIENT_UI_APP_NAME" -DskipTests="$SKIP_TESTS" -Dspring-boot.build-image.imageName="$MOVIE_CLIENT_UI_DOCKER_IMAGE_NAME"
  ./mvnw -Pnative clean spring-boot:build-image --projects "$MOVIE_CLIENT_SHELL_APP_NAME" -DskipTests="$SKIP_TESTS" -Dspring-boot.build-image.imageName="$MOVIE_CLIENT_SHELL_DOCKER_IMAGE_NAME"
else
